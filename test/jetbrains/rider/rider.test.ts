...
describe('Rider integration tests', async () => {
  ...
  it('Can install Rider', { timeout: 600_000 }, async () => {
    await PluginTester.fullTest(pluginPath, [{ type: 'rider' }], {
      validateApply: async () => {
        if (Utils.isMacOS()) {
          const stat = await fs.lstat('/Applications/Rider.app');
          expect(stat.isDirectory()).to.be.true;
        } else {
          const { data } = await testSpawn('which rider');
          expect(data?.trim()).to.include('rider');
        }
      },
      validateDestroy: async () => {
        if (Utils.isMacOS()) {
          const exists = await fs.access('/Applications/Rider.app').then(() => true).catch(() => false);
          expect(exists).to.be.false;
        } else {
          const { data } = await testSpawn('which rider');
          expect(data?.trim() ?? '').not.to.include('rider');
        }
      },
    });
  });
  ...
  it('Can manage JVM heap size', { timeout: 600_000 }, async () => {
    ...
    const findVmOptions = async (): Promise<string | null> => {
      try {
        ...
        const dir = entries.filter((e) => e.startsWith('Rider')).sort().pop();
        if (!dir) return null;
        return path.join(configParent, dir, 'rider.vmoptions');
      } catch {
        return null;
      }
    };
