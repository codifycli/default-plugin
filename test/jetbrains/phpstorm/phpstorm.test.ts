...
describe('PhpStorm integration tests', async () => {
  ...
  it('Can install PhpStorm', { timeout: 600_000 }, async () => {
    await PluginTester.fullTest(pluginPath, [{ type: 'phpstorm' }], {
      validateApply: async () => {
        if (Utils.isMacOS()) {
          const stat = await fs.lstat('/Applications/PhpStorm.app');
          expect(stat.isDirectory()).to.be.true;
        } else {
          const { data } = await testSpawn('which phpstorm');
          expect(data?.trim()).to.include('phpstorm');
        }
      },
      validateDestroy: async () => {
        if (Utils.isMacOS()) {
          const exists = await fs.access('/Applications/PhpStorm.app').then(() => true).catch(() => false);
          expect(exists).to.be.false;
        } else {
          const { data } = await testSpawn('which phpstorm');
          expect(data?.trim() ?? '').not.to.include('phpstorm');
        }
      },
    });
  });
  ...
  it('Can manage JVM heap size', { timeout: 600_000 }, async () => {
    ...
    const findVmOptions = async (): Promise<string | null> => {
      try {
        ...
        const dir = entries.filter((e) => e.startsWith('PhpStorm')).sort().pop();
        if (!dir) return null;
        return path.join(configParent, dir, 'phpstorm.vmoptions');
      } catch {
        return null;
      }
    };
