...
describe('RustRover integration tests', async () => {
  ...
  it('Can install RustRover', { timeout: 600_000 }, async () => {
    await PluginTester.fullTest(pluginPath, [{ type: 'rustrover' }], {
      validateApply: async () => {
        if (Utils.isMacOS()) {
          const stat = await fs.lstat('/Applications/RustRover.app');
          expect(stat.isDirectory()).to.be.true;
        } else {
          const { data } = await testSpawn('which rustrover');
          expect(data?.trim()).to.include('rustrover');
        }
      },
      validateDestroy: async () => {
        if (Utils.isMacOS()) {
          const exists = await fs.access('/Applications/RustRover.app').then(() => true).catch(() => false);
          expect(exists).to.be.false;
        } else {
          const { data } = await testSpawn('which rustrover');
          expect(data?.trim() ?? '').not.to.include('rustrover');
        }
      },
    });
  });
  ...
  it('Can manage JVM heap size', { timeout: 600_000 }, async () => {
    ...
    const findVmOptions = async (): Promise<string | null> => {
      try {
        ...
        const dir = entries.filter((e) => e.startsWith('RustRover')).sort().pop();
        if (!dir) return null;
        return path.join(configParent, dir, 'rustrover.vmoptions');
      } catch {
        return null;
      }
    };
