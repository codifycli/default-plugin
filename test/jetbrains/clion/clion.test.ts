...
describe('CLion integration tests', async () => {
  ...
  it('Can install CLion', { timeout: 600_000 }, async () => {
    await PluginTester.fullTest(pluginPath, [{ type: 'clion' }], {
      validateApply: async () => {
        if (Utils.isMacOS()) {
          const stat = await fs.lstat('/Applications/CLion.app');
          expect(stat.isDirectory()).to.be.true;
        } else {
          const { data } = await testSpawn('which clion');
          expect(data?.trim()).to.include('clion');
        }
      },
      validateDestroy: async () => {
        if (Utils.isMacOS()) {
          const exists = await fs.access('/Applications/CLion.app').then(() => true).catch(() => false);
          expect(exists).to.be.false;
        } else {
          const { data } = await testSpawn('which clion');
          expect(data?.trim() ?? '').not.to.include('clion');
        }
      },
    });
  });
  ...
  it('Can manage JVM heap size', { timeout: 600_000 }, async () => {
    ...
    const findVmOptions = async (): Promise<string | null> => {
      try {
        ...
        const dir = entries.filter((e) => e.startsWith('CLion')).sort().pop();
        if (!dir) return null;
        return path.join(configParent, dir, 'clion.vmoptions');
      } catch {
        return null;
      }
    };
