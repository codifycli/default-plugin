...
describe('IntelliJ IDEA integration tests', async () => {
  ...
  it('Can install IntelliJ IDEA', { timeout: 600_000 }, async () => {
    await PluginTester.fullTest(pluginPath, [{ type: 'intellij-idea' }], {
      validateApply: async () => {
        if (Utils.isMacOS()) {
          const stat = await fs.lstat('/Applications/IntelliJ IDEA.app');
          expect(stat.isDirectory()).to.be.true;
        } else {
          const { data } = await testSpawn('which intellij-idea-community');
          expect(data?.trim()).to.include('intellij-idea-community');
        }
      },
      validateDestroy: async () => {
        if (Utils.isMacOS()) {
          const exists = await fs.access('/Applications/IntelliJ IDEA.app').then(() => true).catch(() => false);
          expect(exists).to.be.false;
        } else {
          const { data } = await testSpawn('which intellij-idea-community');
          expect(data?.trim() ?? '').not.to.include('intellij-idea-community');
        }
      },
    });
  });
  ...
  it('Can manage JVM heap size', { timeout: 600_000 }, async () => {
    ...
    const findVmOptions = async (): Promise<string | null> => {
      try {
        ...
        const dir = entries.filter((e) => e.startsWith('IntelliJIdea')).sort().pop();
        if (!dir) return null;
        return path.join(configParent, dir, 'idea.vmoptions');
      } catch {
        return null;
      }
    };
