...
describe.skipIf(!process.env.JETBRAINS_MANUAL)('PyCharm integration tests', async () => {
  ...
  it('Can install PyCharm', { timeout: 600_000 }, async () => {
    await PluginTester.fullTest(pluginPath, [{ type: 'pycharm' }], {
      validateApply: async () => {
        if (Utils.isMacOS()) {
          const stat = await fs.lstat('/Applications/PyCharm.app');
          expect(stat.isDirectory()).to.be.true;
        } else {
          const { data } = await testSpawn('which pycharm-community');
          expect(data?.trim()).to.include('pycharm-community');
        }
      },
      validateDestroy: async () => {
        if (Utils.isMacOS()) {
          const exists = await fs.access('/Applications/PyCharm.app').then(() => true).catch(() => false);
          expect(exists).to.be.false;
        } else {
          const { data } = await testSpawn('which pycharm-community');
          expect(data?.trim() ?? '').not.to.include('pycharm-community');
        }
      },
    });
  });
  ...
  it('Can manage JVM heap size', { timeout: 600_000 }, async () => {
    ...
    const findVmOptions = async (): Promise<string | null> => {
      try {
        ...
        const dir = entries.filter((e) => e.startsWith('PyCharm')).sort().pop();
        if (!dir) return null;
        return path.join(configParent, dir, 'pycharm.vmoptions');
      } catch {
        return null;
      }
    };
