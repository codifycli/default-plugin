...
const schema = z
  .object({
    settingsZip: z
      .string()
      .optional()
      .describe('Absolute path to a Rider settings ZIP file to import on first install.'),
    importSettings: z
      .boolean()
      .optional()
      .describe(
    ...
    jvmMaxHeapSize: z
      .string()
      .optional()
      .describe('Maximum JVM heap size for Rider, e.g. "2048m" for 2 GB. Defaults to the IDE default (~2 GB).'),
    jvmMinHeapSize: z
      .string()
      .optional()
      .describe('Initial JVM heap size for Rider, e.g. "512m". Defaults to the IDE default.'),
  })
  .meta({ $comment: 'https://codifycli.com/docs/resources/rider' })
  .describe('Install and configure JetBrains Rider IDE with plugins and JVM settings.');

export type RiderConfig = z.infer<typeof schema>;

const defaultConfig: Partial<RiderConfig> = {
  plugins: [],
};

const exampleBasic: ExampleConfig = {
  title: 'Rider with GitHub Copilot and Docker plugins',
  description:
    'Install Rider and add the GitHub Copilot and Docker integration plugins for a modern .NET development workflow.',
  configs: [
    {
      type: 'rider',
...
const exampleAdvanced: ExampleConfig = {
  title: 'Rider with tuned JVM and imported settings',
  description:
    'Install Rider, import previous settings from a ZIP, and increase the heap to 4 GB for large projects.',
  configs: [
    {
      type: 'rider',
...
export class RiderResource extends Resource<RiderConfig> {
  getSettings(): ResourceSettings<RiderConfig> {
    return {
      id: 'rider',
      ...
      parameterSettings: {
        ...
        plugins: { type: 'stateful', definition: new JetBrainsPluginsParameter<RiderConfig>(PRODUCT), order: 1 },
  ...
  override async refresh(parameters: Partial<RiderConfig>): Promise<Partial<RiderConfig> | null> {
    const installed = await isInstalled(PRODUCT);
    if (!installed) return null;

    const result: Partial<RiderConfig> = {};
  ...
  override async create(plan: CreatePlan<RiderConfig>): Promise<void> {
    if (Utils.isMacOS()) {
  ...
  override async modify(pc: ParameterChange<RiderConfig>, plan: ModifyPlan<RiderConfig>): Promise<void> {
  ...
  override async destroy(plan: DestroyPlan<RiderConfig>): Promise<void> {
  ...
  private async importSettingsZip(settingsZip: string): Promise<void> {
    ...
    const configDir = await getOrCreateConfigDir(PRODUCT);
    if (!configDir) {
      throw new Error('Cannot determine Rider config directory for settings import.');
    }
