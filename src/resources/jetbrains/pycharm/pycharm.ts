...
const PRODUCT: JetBrainsProductInfo = {
  macAppName: 'PyCharm',
  macBinaryName: 'pycharm',
  configDirPrefix: 'PyCharm',
  caskName: 'pycharm',
  snapName: 'pycharm-community',
  linuxCommand: 'pycharm-community',
};
...
const schema = z
  .object({
    settingsZip: z
      .string()
      .optional()
      .describe('Absolute path to a PyCharm settings ZIP file to import on first install.'),
    importSettings: z
      .boolean()
      .optional()
      .describe(
    ...
    plugins: z
      ...
      .describe(
        ...
        '(e.g. "com.intellij.ml.llm", "Pythonid"). ' +
        'Plugin IDs can be found on the plugin page under Additional Information.'
      ),
    jvmMaxHeapSize: z
      .string()
      .optional()
      .describe('Maximum JVM heap size for PyCharm, e.g. "2048m" for 2 GB. Defaults to the IDE default (~2 GB).'),
    jvmMinHeapSize: z
      .string()
      .optional()
      .describe('Initial JVM heap size for PyCharm, e.g. "512m". Defaults to the IDE default.'),
  })
  .meta({ $comment: 'https://codifycli.com/docs/resources/pycharm' })
  .describe('Install and configure JetBrains PyCharm IDE with plugins and JVM settings.');

export type PyCharmConfig = z.infer<typeof schema>;

const defaultConfig: Partial<PyCharmConfig> = {
  plugins: [],
};

const exampleBasic: ExampleConfig = {
  title: 'PyCharm with data science and Docker plugins',
  description:
    'Install PyCharm and add the Jupyter notebook support and Docker integration plugins for a data science workflow.',
  configs: [
    {
      type: 'pycharm',
      plugins: ['intellij.jupyter', 'Docker'],
    },
  ],
};

const exampleAdvanced: ExampleConfig = {
  title: 'PyCharm with tuned JVM and imported settings',
  description:
    'Install PyCharm, import previous settings from a ZIP, and increase the heap to 4 GB for large projects.',
  configs: [
    {
      type: 'pycharm',
...
export class PyCharmResource extends Resource<PyCharmConfig> {
  getSettings(): ResourceSettings<PyCharmConfig> {
    return {
      id: 'pycharm',
      ...
      parameterSettings: {
        ...
        plugins: { type: 'stateful', definition: new JetBrainsPluginsParameter<PyCharmConfig>(PRODUCT), order: 1 },
  ...
  override async refresh(parameters: Partial<PyCharmConfig>): Promise<Partial<PyCharmConfig> | null> {
    const installed = await JetBrainsCommon.isInstalled(PRODUCT);
    if (!installed) return null;

    const result: Partial<PyCharmConfig> = {};
  ...
  override async create(plan: CreatePlan<PyCharmConfig>): Promise<void> {
    if (Utils.isMacOS()) {
  ...
  override async modify(pc: ParameterChange<PyCharmConfig>, plan: ModifyPlan<PyCharmConfig>): Promise<void> {
  ...
  override async destroy(plan: DestroyPlan<PyCharmConfig>): Promise<void> {
  ...
  private async importSettingsZip(settingsZip: string): Promise<void> {
    ...
    const configDir = await JetBrainsCommon.getOrCreateConfigDir(PRODUCT);
    if (!configDir) {
      throw new Error('Cannot determine PyCharm config directory for settings import.');
    }
