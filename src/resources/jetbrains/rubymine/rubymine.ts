...
const PRODUCT: JetBrainsProductInfo = {
  macAppName: 'RubyMine',
  macBinaryName: 'rubymine',
  configDirPrefix: 'RubyMine',
  caskName: 'rubymine',
  snapName: 'rubymine',
  linuxCommand: 'rubymine',
};
...
const schema = z
  .object({
    settingsZip: z
      .string()
      .optional()
      .describe('Absolute path to a RubyMine settings ZIP file to import on first install.'),
    importSettings: z
      .boolean()
      .optional()
      .describe(
    ...
    jvmMaxHeapSize: z
      .string()
      .optional()
      .describe('Maximum JVM heap size for RubyMine, e.g. "2048m" for 2 GB. Defaults to the IDE default (~2 GB).'),
    jvmMinHeapSize: z
      .string()
      .optional()
      .describe('Initial JVM heap size for RubyMine, e.g. "512m". Defaults to the IDE default.'),
  })
  .meta({ $comment: 'https://codifycli.com/docs/resources/rubymine' })
  .describe('Install and configure JetBrains RubyMine IDE with plugins and JVM settings.');

export type RubyMineConfig = z.infer<typeof schema>;

const defaultConfig: Partial<RubyMineConfig> = {
  plugins: [],
};

const exampleBasic: ExampleConfig = {
  title: 'RubyMine with GitHub Copilot and Docker plugins',
  description:
    'Install RubyMine and add the GitHub Copilot and Docker integration plugins for a modern Ruby on Rails development workflow.',
  configs: [
    {
      type: 'rubymine',
...
const exampleAdvanced: ExampleConfig = {
  title: 'RubyMine with tuned JVM and imported settings',
  description:
    'Install RubyMine, import previous settings from a ZIP, and increase the heap to 4 GB for large projects.',
  configs: [
    {
      type: 'rubymine',
      settingsZip: '/path/to/rubymine-settings.zip',
      importSettings: true,
...
export class RubyMineResource extends Resource<RubyMineConfig> {
  getSettings(): ResourceSettings<RubyMineConfig> {
    return {
      id: 'rubymine',
      ...
      parameterSettings: {
        ...
        plugins: { type: 'stateful', definition: new JetBrainsPluginsParameter<RubyMineConfig>(PRODUCT), order: 1 },
  ...
  override async refresh(parameters: Partial<RubyMineConfig>): Promise<Partial<RubyMineConfig> | null> {
    const installed = await isInstalled(PRODUCT);
    if (!installed) return null;

    const result: Partial<RubyMineConfig> = {};
  ...
  override async create(plan: CreatePlan<RubyMineConfig>): Promise<void> {
    if (Utils.isMacOS()) {
  ...
  override async modify(pc: ParameterChange<RubyMineConfig>, plan: ModifyPlan<RubyMineConfig>): Promise<void> {
  ...
  override async destroy(plan: DestroyPlan<RubyMineConfig>): Promise<void> {
  ...
  private async importSettingsZip(settingsZip: string): Promise<void> {
    ...
    const configDir = await getOrCreateConfigDir(PRODUCT);
    if (!configDir) {
      throw new Error('Cannot determine RubyMine config directory for settings import.');
    }
