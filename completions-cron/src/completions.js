...
async function loadPyenvPythonVersions() {
    ...
    const pyenvVersions = await response.json();
    return pyenvVersions.map((v) => v.name);
}

async function loadNvmNodeVersions() {
    const response = await fetch('https://nodejs.org/dist/index.json');
    const nodeVersions = await response.json();
...
async function loadHomebrewFormulae() {
    const response = await fetch('https://formulae.brew.sh/api/formula.json');
    const data = await response.json();
    return data.map((d) => d.name);
}

async function loadHomebrewCasks() {
    const response = await fetch('https://formulae.brew.sh/api/cask.json');
    const data = await response.json();
    return [...new Set(data.flatMap((d) => d.full_token))];
}

async function loadAptPackages() {
    return [];
}

const completionModules = [
  { resourceType: "pyenv", parameterPath: "/pythonVersions", fetch: loadPyenvPythonVersions },
  { resourceType: "nvm", parameterPath: "/nodeVersions", fetch: loadNvmNodeVersions },
  { resourceType: "homebrew", parameterPath: "/formulae", fetch: loadHomebrewFormulae },
  { resourceType: "homebrew", parameterPath: "/casks", fetch: loadHomebrewCasks },
  { resourceType: "apt", parameterPath: "/install", fetch: loadAptPackages },
];

const BATCH_SIZE = 1000;
async function getResourceId(supabase, resourceType, cache) {
...
async function processModule(supabase, resourceType, parameterPath, fetchFn, resourceIdCache) {
    console.log(`Processing ${resourceType}${parameterPath}...`);
    const values = await fetchFn();
    console.log(`  Fetched ${values.length} values`);
    ...
    console.log(`  Done: inserted ${values.length} completions`);
}
var orchestrator = {
    async fetch(req) {
    ...
    async scheduled(_event, env, _ctx) {
      console.log('scheduled')
        ...
        const resourceIdCache = new Map();
        const results = await Promise.allSettled(completionModules.map(({ resourceType, parameterPath, fetch }) => processModule(supabase, resourceType, parameterPath, fetch, resourceIdCache)));
